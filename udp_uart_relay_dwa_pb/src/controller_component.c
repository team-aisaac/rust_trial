...
void robot_wrap_kick(State *next_goal_pose, TrackedBall ball, State r_ball, State ball_goal, TrackedRobot my_robot, double *circumferential_error,double *radius_error, double *goal_theta,
    bool *ball_kick_con_flag, const unsigned int robot_id, float *kick_con_max_velocity_theta, bool free_kick_flag, int32_t ball_target_allowable_error, 
  ...
  float ball_robot_distance = hypot(wrap_ball.r_ball_x - ROBOT_KICK_MIN_X, wrap_ball.r_ball_y);
  //RCLCPP_INFO(rclcpp::get_logger("dwa"),"distance : %f", ball_robot_distance);
  
  if(WRAP_KICK_CONTROL_CHANGE_DISTANCE < ball_robot_distance){
    (*ball_kick_con_flag) = false;
  } else {
    (*ball_kick_con_flag) = true;
...
void dribble(State dribble_goal, TrackedBall ball, State r_ball, TrackedRobot my_robot, State *next_goal_pose, bool *dribble_con_flag, 
    const unsigned int robot_id, int32_t dribble_complete_distance, micon_trape_con *dribble_trape_c, bool *dribble_ball_move_flag, 
  ...
  double ball_dribble_goal_distance = 1000 * hypot(dribble_goal.x - ball.pos.x, dribble_goal.y - ball.pos.y);
  double ball_robot_distance = hypot(1000 * r_ball.x - ROBOT_KICK_MIN_X, 1000 * r_ball.y);
  if(ball_dribble_goal_distance < dribble_complete_distance || ROBOT_DRIBBLE_DISTANCE < ball_robot_distance){
    ...
    (*dribble_con_flag) = false;
    (*dribble_ball_move_flag) = false;
    return;
  }
  ...
  bool pre_dribble_ball_move_flag = *dribble_ball_move_flag;
  (*dribble_ball_move_flag) = dribble_wrap_motion(drible_robot, dribble_ball, &pid, dribble_complete_distance, *dribble_ball_move_flag, dribble_active);
  ...
  if( *dribble_ball_move_flag ){
    ...
    (*dribble_active) = true;
    if(pre_dribble_ball_move_flag == false) {
      //RCLCPP_INFO(rclcpp::get_logger("dribble"),"trape con start");
      micon_trapezoidal_DWA_change(drible_robot.w_robot_x, drible_robot.w_robot_y, drible_robot.w_robot_vx, drible_robot.w_robot_vy, dribble_trape_c, 
    ...
    for(uint16_t i = 0; i < RASPI_TIME_STEP/MICON_TIME_STEP; i++){
      micon_trapezoidal_dribble_control(dribble_ball.target_ball_x, dribble_ball.target_ball_y, dribble_trape_c, dribble_ball.w_ball_x, dribble_ball.w_ball_y);
      pid.robot_goal_theta = dribble_move_deg(dribble_ball.r_ball_y, pid.robot_goal_theta);
    }

    next_goal_pose->x = dribble_trape_c->virtual_x/1000;    //単位変換(mm -> m)
    next_goal_pose->y = dribble_trape_c->virtual_y/1000;    //単位変換(mm -> m)
  }   
