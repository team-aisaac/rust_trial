...
#define MARGIN_DISTANCE 20      //Margin distance when avoiding obstacles
#define COLLISION_CHECK_MARGIN_DISTANCE 30      //Margin distance when avoiding obstacles
...
#define DELTA_BIG_DEG 5      //探索時の粗目の時の刻み幅(deg)
#define DELTA_BIG_ACCEL 500  //探索時の粗目の時の刻み幅(mm/s^2)
#define DELTA_FINE_DEG 0.1  //探索時の細かめの時の刻み幅(deg)
#define DELTA_FINE_ACCEL 100 //探索時の細かめの時の刻み幅(mm/s^2)
...
#define FINE_WEIGHT_ANGLE_MIDLE_TARGET 0.4   //探索時の細かめの時の中間目標点に向かう角度の重み
#define FINE_WEIGHT_VELO_MIDLE_TARGET 0.1    //探索時の細かめの時の中間目標点に向かう速度の重み
#define FINE_WEIGHT_ANGLE_MIDLE_F_TARGET 0.5   //探索時の細かめの時の最終目標点に向かう角度の重み
...
void execDWA(   int32_t x, int32_t y, int32_t theta, int32_t Vx, int32_t Vy, int32_t omega, int32_t *targetX, int32_t *targetY, int32_t targetTheta, int32_t *midle_targetX, int32_t *midle_targetY,
                ...
                bool *is_enable, bool *path_enable, bool *prohidited_zone_start, int32_t *output_x, int32_t *output_y, int32_t *output_omega, int32_t *output_ax, int32_t *output_ay);
//フィールドデータを格納
//void field_data();
//最大値を算出
